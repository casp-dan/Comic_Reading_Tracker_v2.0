...
async function getSeriesList(test) {
    try {
        let response = await axios.post(
            `http://127.0.0.1:${port}/seriesList`,
            test
        );
        let x = response.data;
...
async function getCreatorList(_event, role) {
    try {
        const response = await fetch(
            `http://127.0.0.1:${port}/creatorList?role=${role}`,
        );
...
async function getSeriesEntries(_event, seriesName) {
    try {
        const response = await fetch(
            `http://127.0.0.1:${port}/seriesEntries?seriesName=${seriesName}`
        );
        const data = await response.text();
        let rep = data.replaceAll('"', "'");
        rep = rep.replaceAll("[('", "");
...
async function getDateEntries(_event, inDate) {
    try {
        ...
        const response = await fetch(
            `http://127.0.0.1:${port}/dateEntries?date=${date}`
        );
        const data = await response.text();
        let outArr = [];
        if (data !== "[]") {
...
async function getCreatorEntries(_event, nameRole) {
    try {
        ...
        let role=nameRole[1]
        const response = await fetch(
            `http://127.0.0.1:${port}/creatorEntries?creator=${name}&role=${role}`
        );
        const data = await response.text();
        let outArr = [];
        if (data !== "[]") {
...
async function getYearlyStats(_event, year) {
    try {
        const response = await fetch(
            `http://127.0.0.1:${port}/yearlyStats?year=${year}`
        );
        const data = await response.text();
        let rep = data.replaceAll("[", "");
        rep = rep.replaceAll("]", "");
...
async function getMonthlyStats(_event, monthYear) {
    try {
        ...
        const response = await fetch(
            `http://127.0.0.1:${port}/monthlyStats?year=${year}&month=${month}`
        );
        const data = await response.text();
        let rep = data.replaceAll("[", "");
        rep = rep.replaceAll("]", "");
...
async function getSnapshotStats(_event, startEnd) {
    try {
        ...
        let end = startEnd[1];
        const response = await fetch(
            `http://127.0.0.1:${port}/snapshotStats?start=${start}&end=${end}`
        );
        const data = await response.text();
        let rep = data.replaceAll("[", "");
        rep = rep.replaceAll("]", "");
...
async function createSeries(_event, values) {
    ...
    if (currentSeriesID === null) {
        response = await fetch(
            `http://127.0.0.1:${port}/createSeries?seriesName=${seriesName}&publisher=${publisher}&xmen=${xmen}&seriesID=None`
        );
    } else {
        response = await fetch(
            `http://127.0.0.1:${port}/createSeries?seriesName=${seriesName}&publisher=${publisher}&xmen=${xmen}&seriesID=${currentSeriesID}`
        );
    }
    const data = await response.text();
...
async function makeSeriesSelectList(searchTerm, publisher, startYear) {
    const response = await fetch(
        `http://127.0.0.1:${port}/selectSeries?seriesName=${searchTerm}&publisher=${publisher}&startYear=${startYear}`
    );
    const data = await response.text();
...
async function getLastDateTime(_event, dateString) {
    const response = await fetch(
        `http://127.0.0.1:${port}/getLastDateTime?dateString=${dateString}`
    );
    const data = await response.text();
...
async function entryExists(_event, values) {
    ...
    const response = await fetch(
        `http://127.0.0.1:${port}/entryExists?series=${series}&issue=${issue}&date=${date}`
    );
    const data = await response.text();
    if (data === "True") {
        return true;
...
async function addIssue(_event, values) {
    ...
    let date = values[3];
    const response = await fetch(
        `http://127.0.0.1:${port}/addIssue?issue=${issue}&series=${series}&xmenAdj=${xmenAdj}&date=${date}`
    );
    const data = await response.text();
    ...
    if (data === "True") {
        ...
        const response2 = await fetch(
            `http://127.0.0.1:${port}/issueAPI?issue=${issue}&series=${series}&year=${startYear}`
        );
        const data2 = await response2.text();
        console.log(`Series ID is: ${data2}`);
    }
...
async function getInfo(_event, values) {
    ...
    let series = values[1];
    const response = await fetch(
        `http://127.0.0.1:${port}/getInfo?issue=${issue}&series=${series}`
    );
    const data = await response.text();
    theObj = {};
    if (data !== "") {
    ...
    const url = await fetch(
        `http://127.0.0.1:${port}/getURL?issue=${issue}&series=${series}`
    );
...
async function addPublisher(publisher) {
    const response = await fetch(
        `http://127.0.0.1:${port}/addPublisher?publisher=${publisher}`
    );
    const data = await response.text();
    console.log(`Issue added: ${data}`);
}
...
function makeSeriesSelectPopup() {
    ...
    seriesSelectWindow.on("close", async () => {
        if (currentSeriesID !== undefined) {
            const response = await fetch(
                `http://127.0.0.1:${port}/getTrueName?seriesID=${currentSeriesID}`
            );
            const data = await response.text();
            console.log(`${data}`);
            let newSeriesName = data;
            win.webContents.send(
                "secondWindow:returnSeriesName",
                newSeriesName
            );
        }
    });
