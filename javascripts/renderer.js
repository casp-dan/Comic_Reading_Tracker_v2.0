    ...
    if (issueField.value.includes(",")) {
        ...
        if (issues.length !== dates.length) {
            window.dialogs.error([
                "Improper Entry",
                "Please ensure each set of issues has a correspondig date!",
            ]);
...
percentBox.addEventListener("change", function () {
    if (this.checked) {
        let totalNum = parseInt(
            document.querySelector(`#${totalValues[0]}`).innerHTML
        );
        let xmenNum = parseInt(
            document.querySelector(
                `#${totalValues[totalValues.indexOf("xmenValue")]}`
            ).innerHTML
        );
        let marvelNum = parseInt(
            document.querySelector(`#${totalValues[2]}`).innerHTML
        );
        ...
        let xmenLabel = document.querySelector(
            `#${totalValues[totalValues.indexOf("xmenValue")]}`
        );
...
async function makeDropdowns(type) {
    if (type === "series") {
        ...
        let html = getDropdown(creatorList);
...
function render(series) {
    const searchSeries = series.filter(function (title) {
        return title
            ...
            .toLowerCase()
            .replaceAll("'", "")
            .includes(seriesField.value.toLowerCase().replaceAll("'", ""));
    });
...
function renderView(series) {
    const searchSeries = series.filter(function (title) {
        return title
            ...
            .replaceAll("'", "")
            .includes(
                seriesFieldViewTab.value.toLowerCase().replaceAll("'", "")
            );
    });
    const htmlTitles = getDropdown(searchSeries);
...
function renderCreatorView(names) {
    ...
    const htmlNames = getDropdown(searchCreator);
...
searchSeriesButton.addEventListener("click", async function () {
    if (seriesField.value === "") {
        ...
        let seriesName = await window.secondWindow.selectSeries([
            seriesField.value,
            publisherDrop.value,
        ]);
...
async function getSeriesID(seriesName, publisher, xmen) {
    let seriesID = await window.secondWindow.selectSeries([
        seriesName,
        publisher,
        xmen,
    ]);
...
async function addBook(entry, series) {
    ...
    window.entries
        .getLastDateTime(date.toSearchString())
        .then(async (lastDate) => {
            ...
            window.dialogs.dialog([
                "Finished",
                `All entries for ${series} completed`,
            ]);
        });
}
...
async function listSeriesViewItems() {
    let issueCount = 0;
    clearSeriesTable();
    const series = seriesList.filter(function (series) {
        return series.toString() === seriesFieldViewTab.value;
    })[0];
...
function createDateViewTable(issueList, issueCount) {
    if (issueList.length === 0) {
        window.dialogs.error([
            "Nothing Read",
            "You did not read any issues on this date!",
        ]);
    } else {
        issueList.forEach(function (item) {
            // const html=getDateHTML(item['SeriesName'],item['IssueName'],item['DateString'],item['coverURL']);
            console.log(item);
            const html = getDateHTML(item[1], item[0], item[3], item[2]);
...
async function getSnapshotStats(startDate, endDate) {
    let totals = await window.stats.snapshotStats([
        startDate.toSearchString(),
        endDate.toSearchString(),
    ]);
    let pubList = await window.dropdownList.pubList();
    setValues(pubList, totals);
}
...
const entryFactory = (seriesName, issues, date, publisher, xmen, xmenAdj) => {
    return {
        ...
        makeIssueList() {
            if (
                this._issues.includes("-") &&
                this._issues.split("-")[0] != ""
            ) {
