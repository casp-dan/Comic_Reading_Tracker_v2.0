...
@app.route("/creatorList", methods = ['GET', 'POST', 'OPTIONS'] )
def getCreatorNames():
    if request.method == 'GET':
    ...
    try:
        ...
        mydb = mysql.connect()
        mycursor = mydb.cursor()
        sql=f"SELECT {role}Name from {role}"
...
@app.route("/creatorEntries", methods = ['GET', 'POST', 'OPTIONS'] )
def getCreatorEntries():
    if request.method == 'GET':
            creator = request.args.get('creator')
            role = request.args.get('role')
    # return  seriesName
    creatorName=creator.replace("\'","\\'")
    mydb = mysql.connect()
    mycursor = mydb.cursor()
    sql=f"select e.IssueName,e.SeriesName,i.coverURL,e.DateString FROM Entry e, RealIssue i, Issue{role} c where c.{role}Name='{str(creatorName)}' and c.IssueName=i.IssueName and i.IssueName=e.IssueName and c.SeriesName=i.SeriesName and i.SeriesName=e.SeriesName order by DateString"
    mycursor.execute(sql)
...
def issueAPI():
    ...
    try:
        ...
        if seriesID is not None:
            roles=["Writer","Colorist","Penciller","Inker","Artist","Story","Plot","Script","Illustrator"]
        ...
        return str((status,issueList,mok_user,mok_pass))
